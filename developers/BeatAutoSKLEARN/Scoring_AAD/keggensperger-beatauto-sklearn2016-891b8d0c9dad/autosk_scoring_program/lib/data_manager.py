...
class DataManager:
    ...
    def __init__(self, basename="", input_dir="", max_samples=float('inf')):
        '''Constructor'''
    ...
    def loadData (self, filename):
        ...
        if self.use_pickle and os.path.exists (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle")):
            with open (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle"), "r") as pickle_file:
                logger.info("Loading pickle file : " + os.path.join(self.tmp_dir, 
        ...
        if self.use_pickle:
            with open (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle"), "wb") as pickle_file:
                logger.info("Saving pickle file : " + os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle"))
                p = pickle.Pickler(pickle_file) 
                p.fast = True 
                p.dump(data)
        end = time.time()
        logger.info( "[+] Success in %5.2f sec" % (end - start))
        return data
    ...
    def loadLabel (self, filename, verbose=True):
        ...
        if self.use_pickle and os.path.exists (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle")):
            with open (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle"), "r") as pickle_file:
                logger.info("Loading pickle file : " +
							os.path.join(self.tmp_dir, os.path.basename(filename) + ".pickle"))
                return pickle.load(pickle_file)
        ...
        if self.use_pickle:
            with open (os.path.join (self.tmp_dir, os.path.basename(filename) + ".pickle"), "wb") as pickle_file:
                logger.info(verbose, "Saving pickle file : " +
							os.path.join(self.tmp_dir, os.path.basename(filename) + ".pickle"))
                p = pickle.Pickler(pickle_file) 
                p.fast = True 
                p.dump(label)
    ...
    def getInfo (self, filename):
        ...
        if os.path.exists(filename):
            ...
            logger.info("Info file found : " + os.path.abspath(filename))
